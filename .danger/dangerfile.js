...
const validatePRSize = () => {
    const additions = danger_1.danger.github.pr.additions;
    const deletions = danger_1.danger.github.pr.deletions;
    const totalChanges = additions + deletions;
    const fileCount = danger_1.danger.git.modified_files.length +
        danger_1.danger.git.created_files.length +
        danger_1.danger.git.deleted_files.length;
    ...
    if (totalChanges > CONFIG.PR_SIZE.XLARGE) {
        (0, danger_1.fail)(`🔴 **PR too large (${totalChanges} lines)**
...
**Action Required**: Break this into smaller, focused PRs`);
        ...
        (0, danger_1.warn)(`⚠️ **Large PR (${totalChanges} lines)** - Consider splitting
...
**Finding**: PRs >600 lines have 2x higher defect rate`);
        ...
        (0, danger_1.message)(`📊 PR Size: ${totalChanges} lines (Medium) - Acceptable`);
    }
    else {
        (0, danger_1.message)(`✅ PR Size: ${totalChanges} lines (Small) - Ideal for review`);
    }
};
/**
...
const validateTestCoverage = async () => {
    const hasTestChanges = danger_1.danger.git.modified_files.some(f => f.includes('.spec.') || f.includes('.test.') || f.includes('__tests__'));
    const hasCodeChanges = danger_1.danger.git.modified_files.some(f => (f.endsWith('.ts') || f.endsWith('.tsx')) &&
        !f.includes('.spec.') &&
        !f.includes('.test.'));
    if (hasCodeChanges && !hasTestChanges) {
        (0, danger_1.fail)(`🔴 **No tests added/modified**
...
**Action Required**: Add tests for changed code`);
    ...
    try {
        const coverageReport = (0, child_process_1.execSync)('yarn test --coverage --silent --json', {
            encoding: 'utf8',
        });
        ...
        if (coverage.coverageMap) {
            ...
            if (summary.lines.pct < CONFIG.COVERAGE.MIN_GLOBAL) {
                (0, danger_1.fail)(`🔴 **Coverage below minimum: ${summary.lines.pct}%**
...
"High performers have 2x better coverage"`);
        ...
        (0, danger_1.message)('⚠️ Could not determine test coverage - run locally to verify');
    }
};
/**
...
const validateSecurity = () => {
    const files = [...danger_1.danger.git.modified_files, ...danger_1.danger.git.created_files];
    files.forEach(file => {
        if (file.endsWith('.ts') || file.endsWith('.tsx') || file.endsWith('.js')) {
            ...
            secretPatterns.forEach(pattern => {
                ...
                if (match) {
                    (0, danger_1.fail)(`🔴 **Potential hardcoded secret in ${file}**
...
**Action Required**: Use environment variables or secrets management`);
            ...
            if (content.includes('console.log') &&
                !file.includes('.test.') &&
                !file.includes('.spec.')) {
                (0, danger_1.warn)(`⚠️ **console.log found in ${file}**
...
**Recommendation**: Use proper logging framework with levels`);
            ...
            if (content.includes('eval(') || content.includes('Function(')) {
                (0, danger_1.fail)(`🔴 **Dangerous eval() usage in ${file}**
...
const validateDependencies = () => {
    const packageChanged = danger_1.danger.git.modified_files.includes('package.json');
    const lockChanged = danger_1.danger.git.modified_files.includes('yarn.lock');
    if (packageChanged && !lockChanged) {
        (0, danger_1.fail)(`🔴 **package.json modified but yarn.lock not updated**
...
**Action Required**: Run \`yarn install\` and commit yarn.lock`);
    ...
    if (packageChanged) {
        ...
        try {
            const auditResult = (0, child_process_1.execSync)('yarn audit --level high --json', {
                encoding: 'utf8',
            });
            const audit = JSON.parse(auditResult);
            if (audit.summary && audit.summary.total > 0) {
                (0, danger_1.fail)(`🔴 **Security vulnerabilities in dependencies**
...
const validateDocumentation = () => {
    const hasDocsChanges = danger_1.danger.git.modified_files.some(f => f.includes('README') || f.includes('CHANGELOG') || f.endsWith('.md'));
    const isFeature = danger_1.danger.github.pr.title.toLowerCase().includes('feat') ||
        danger_1.danger.github.pr.title.toLowerCase().includes('add');
    if (isFeature && !hasDocsChanges) {
        (0, danger_1.warn)(`⚠️ **No documentation updates for new feature**
...
**Recommendation**: Update README or add to docs/`);
    ...
    if (!danger_1.danger.git.modified_files.includes('CHANGELOG.md')) {
        const isFix = danger_1.danger.github.pr.title.toLowerCase().includes('fix');
        const isBreaking = danger_1.danger.github.pr.title.toLowerCase().includes('breaking');
        if (isFeature || isFix || isBreaking) {
            (0, danger_1.warn)(`⚠️ **No CHANGELOG entry**
...
const validateTypeScript = async () => {
    const tsFiles = danger_1.danger.git.modified_files.filter(f => f.endsWith('.ts') || f.endsWith('.tsx'));
    if (tsFiles.length > 0) {
        try {
            const tscResult = (0, child_process_1.execSync)('yarn tsc --noEmit', { encoding: 'utf8' });
            (0, danger_1.message)('✅ TypeScript compilation successful');
        }
        catch (error) {
            (0, danger_1.fail)(`🔴 **TypeScript compilation failed**
...
**Action Required**: Fix type errors before merge`);
        ...
        tsFiles.forEach(file => {
            ...
            if (anyCount > 0) {
                (0, danger_1.warn)(`⚠️ **${anyCount} \`any\` types in ${file}**
...
const validatePerformance = () => {
    ...
    try {
        ...
        if (bundleStats.size > 3 * 1048576) {
            // 3MB
            (0, danger_1.fail)(`🔴 **Bundle size too large: ${sizeMB}MB**
...
**Action Required**: Code split or lazy load`);
            ...
            // 2MB
            (0, danger_1.warn)(`⚠️ Bundle size: ${sizeMB}MB - Consider optimization`);
        }
    }
    catch (e) {
...
const runValidations = async () => {
    (0, danger_1.markdown)(`## 🤖 Automated Code Review Report
