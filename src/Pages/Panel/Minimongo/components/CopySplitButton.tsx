...
import {
  ...
  CopyFormatKey,
  generateByKey,
} from '@/Pages/Panel/Minimongo/services/CopyFormats'
...
export const CopySplitButton: React.FC<Props> = ({
  collectionName,
  doc,
  initialFormat = 'raw',
}) => {
    ...
    <Menu>
      {COPY_FORMATS.map(({ key, label, description }) => (
        <MenuItem
          ...
          text={label}
          labelElement={
            <span style={{ color: '#8A9BA8', fontSize: 11 }}>
              {description}
            </span>
          }
          onClick={async () => {
  ...
  return (
    <ButtonGroup>
      <Button icon='clipboard' onClick={onPrimary}>
        {primaryLabel}
      </Button>
